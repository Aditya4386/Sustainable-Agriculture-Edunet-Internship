# 🌱 Fertilizer Recommendation System  

This repository contains my project for the **Shell–Edunet Skills4Future AICTE Internship** under the **Sustainable Agriculture** domain.  
The goal is to apply **Machine Learning** techniques to recommend the most suitable fertilizer for a given crop, supporting **sustainable farming practices**.  

---

## 📌 Project Title  
**Fertilizer Recommendation System**  

---

## 🎯 Objective  
The aim of this project is to build a system that recommends the right fertilizer for crops based on soil and environmental conditions.  
By doing so, the project contributes to:  
- 🌾 Optimized fertilizer usage  
- 🌻 Improved crop yield  
- 🌍 Reduced environmental impact  
- 🚜 Sustainable and eco-friendly farming practices  

---

## 📊 Dataset  
The dataset used in this project is from Kaggle:  

👉 [Crop and Fertilizer Dataset for Western Maharashtra](https://www.kaggle.com/datasets/sanchitagholap/crop-fertilizer-dataset-for-western-maharashtra)  

It contains information about crops, soil nutrients (NPK), pH, rainfall, temperature, and fertilizer recommendations.  

---

## 🗓️ Week 1 Progress (30% Completion)  
In **Week 1**, the following tasks were completed:  
1. ✅ Selected project title: *Fertilizer Recommendation System*  
2. ✅ Chosen and downloaded the Kaggle dataset  
3. ✅ Performed **initial preprocessing tasks**, including:  
   - Loading and exploring dataset structure  
   - Handling missing/null values  
   - Removing duplicates  
   - Basic encoding for categorical variables  

---

## 🗓️ Week 2 Progress (60% Completion)  
In **Week 2**, the following tasks were completed:  
1. ✅ Performed **Exploratory Data Analysis (EDA)** to understand dataset distributions and correlations  
2. ✅ Created **feature (X) and target (y) splits**  
3. ✅ Applied **train-test split** to prepare the dataset for model training  
4. ✅ Planned ML model building using algorithms like Decision Tree, Random Forest, and XGBoost  

---

## 🔧 Tools & Technologies  
- Python 🐍  
- Pandas & NumPy → Data preprocessing  
- Matplotlib & Seaborn → Data visualization  
- Scikit-learn → Machine Learning models  

---

## 📌 Next Steps (Week 3 Plan – Final Stage)  
- Train and evaluate **multiple ML models**  
- Perform **hyperparameter tuning** for best performance  
- Build **final fertilizer recommendation system**  
- Prepare **PPT presentation** and documentation  
- Optimize and polish codebase for final submission  

---

✅ *This README documents the Week 2 milestone (60% project completion) as per internship requirements. The remaining 40% work includes ML model training, optimization, and presentation preparation.*  
